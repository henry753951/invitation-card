...
const reverseCard = (floating = false) => {
...
<template>
  <TresCanvas v-bind="gl">
    ...
    <OrbitControls
      ...
      :enable-zoom="false"
      ref="controlRef"
      :enable-rotate="!reverse"
    />

    <!-- Card -->
    <TresGroup ref="cardRef" cast-shadow>
      <Suspense cast-shadow>
        <TresModelCard cast-shadow @onload="animateCard" />
      </Suspense>

      <Html transform :position="[0, 0, 0.01]" occlude>
        <div flex flex-col gap-3 z="-10" w-200px h-130px p-3 relative class="back-html">
          ...
          <p absolute bottom-0 left-0 p-2 opacity-80 text-size="0.5rem"># {{ member.id }}</p>
        </div>
      </Html>
    </TresGroup>
